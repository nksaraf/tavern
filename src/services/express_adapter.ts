...
export default class ExpressAdapter extends Service {
  ...
  middleware = async (
    ...
    if (url === undefined || url.pathname === undefined) {
      ...
      const { type, payload, ctx } = await this.ask('HANDLE_REQUEST', {
        req: tRequest
      });
      ...
      const status =
        response.payload.status || (this.isError(response.type) ? 400 : 200);
  ...
  listen = async () => {
    try {
      ...
      return this.tell('LOG', {
        message: `🚀 Serving at http://:::${process.env.PORT || 5000}/`
      });
    } catch (error) {
