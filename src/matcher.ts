...
export class GlobPattern implements Matcher {
  ...
  private static globMatch(
    ...
    if (
      checkArgType(message, 'object', 'message') &&
      checkArgType(message.type, 'string', 'message.type')
    ) {
  ...
  constructor(pattern: string) {
    ...
    this.match = (message: Message | string) =>
      GlobPattern.globMatch(message, this.sub_patterns);
  }
}
